...
### krepp v0.10.0 (untagged, upstream master at `31205033`)

- **Location:** `ext/krepp/` (git submodule on `bo1929/krepp`, the project's own repository; version captured via `git describe --tags --dirty --always` → `KREPP_GIT_VERSION`; what that reads for this untagged pin is in the `place_krepp` Availability note in [`docs/phylogeny.md`](../phylogeny.md), and krepp's own `VERSION` string says `v0.10.0`)
- **Purpose:** k-mer LSH index + maximum pseudo-likelihood phylogenetic placement, exposed as `place_krepp`, and index construction, exposed as `krepp_index_create`. Both go through the same linked krepp, so the version that writes an index is the version that reads it.
- **Why the pin is past the v0.9.1 tag.** Three upstream PRs, all merged, are what make the builder reachable at all: [#11](https://github.com/bo1929/krepp/pull/11) widened `Tree`'s stream API to `std::istream`/`std::ostream`, [#12](https://github.com/bo1929/krepp/pull/12) added `set_error_handler` so `error_exit` no longer has to be `std::exit`, and [#13](https://github.com/bo1929/krepp/pull/13) moved `IndexMultiple` and a plain `IndexConfig` struct out of `krepp.cpp` into `index.cpp`/`index.hpp`. Before #13 the whole builder lived in the one file this build excludes.
...
- **`src/krepp.cpp` is excluded** — it holds `main()` and the CLI11 argument layer. Two consequences worth knowing:
  ...
  It also converts one case krepp cannot: two indexes in one directory. `krepp index` never clears what is already there, so re-indexing with different `-h`/`-w` leaves both file sets behind, and krepp notices only inside its partial loaders (`Index::generate_partial_tree`, `load_partial_tree`, `load_partial_index`: `index.cpp:26`, `:47`, `:86`) — `error_exit`. The filename suffix decides it first, through `krepp_detail::PartialHashConfig`: `m` and `frac` must agree across partials, and so must `r` when `frac` is true, mirroring `LSHF::check_compatible` (`lshf.cpp:163-170`); under `frac := false` the residue is the partial id and is expected to differ.
  ...
  For `krepp_index_create` there is no DuckDB-side parallelism to substitute — a build is one call, not per-row work — so **the index build is genuinely single-threaded**, and the function deliberately exposes no `threads` parameter rather than one that does nothing. Turning krepp's OpenMP back on would need its parallel drivers to capture a per-thread `exception_ptr` first, because a throwing error handler may not unwind out of an OpenMP structured block.
...
- **`_WOPENMP=0 _WLCURL=0`** must likewise match on both sides: they gate declarations in krepp's headers, which `KreppPlacer.cpp` and `KreppIndexBuilder.cpp` include.
...
- **Index layout is version-specific.** v0.9.1 swapped the recursion branches in `Node::generate_tree` back (they had been swapped in v0.9.0), so an index written by a different krepp is misread rather than rejected. This is why `run_tests.sh` stamps the toy index with both the submodule sha and the CLI's version banner. It is also why `.gitmodules` tracking `master` here is a footgun worth knowing about: `git submodule update --remote` would move the pin off the recorded commit without breaking a build, and the toy-index stamp — not the branch — is what actually catches the resulting mismatch. (There is no `v0.9.1` *branch* upstream to pin instead — only `master` and `hm` exist, and v0.9.1 is a tag. Naming a tag there does not work: `git submodule update --remote` resolves `refs/remotes/origin/<branch>` and fails with `Unable to find refs/remotes/origin/<tag> revision`, measured on git 2.50.1. Omitting the key entirely is equivalent to `master`, since git falls back to the remote HEAD.) As of 2026-09-10 bioconda's newest krepp is **0.9.1**, older than this pin, so `conda install bioconda::krepp` does **not** give a binary matching it — build the CLI from source at the pinned commit.
- **`error_exit` — no longer fatal, still pre-checked.** krepp reports failures through `error_exit`, which was `std::exit` and would take the DuckDB process with it. Upstream #12 added `set_error_handler`, and `LoadInternal` installs one (`miint::InstallKreppErrorHandler`, `src/KreppIndexBuilder.cpp`) that throws `miint::KreppFatalError` instead. It is installed at extension load, not lazily, because `error_exit` reads the handler without a lock — installing it once a query might already be inside krepp is a data race. It is safe here only because this build has no OpenMP: an exception may not leave an OpenMP structured block, and with `_WOPENMP=0` and no `-fopenmp` the pragmas are ignored.
  **The pre-validation below is still worth its weight**, and was written before the handler existed: krepp's messages name neither the file nor the offending construct, and several of these shapes are ones other tools emit by default. Losing the process was the original motivation; a usable error message is the remaining one. The reachable cases are wider than the `QSeq` gzopen the `place()` comment names: `Tree::load` and the partial-index loaders reach it via `check_fstream_or_exit` (`common.cpp:29-38`), which is why `SharedKreppIndex`'s constructor pre-validates paths, and `Tree::load` reaches it three further ways, all user-triggerable through `newick_path`, from three different places in krepp:
  ...
  Two further `Tree::load` paths are mirrored the same way: **partial `{N}` decoration** (`phytree.cpp:446-449`) and a **duplicate `{N}` edge number** (`:440-443`, new in v0.9.1). Both throw rather than exit, and both are asserted in `test/sql/place_krepp_toy.test`. Mirroring the decoration check is only sound because the one decoration form miint's parser reads — `{N}` after the branch length, the jplace spec's own — is read identically by krepp; a tree decorated in krepp's other accepted position, `)​{N}:len`, is rejected earlier by miint's parser and is a known narrower-than-krepp limitation.
  **The `{N}` range is ours, not krepp's.** miint's parser reads the decoration as `int64`; krepp narrows it with `static_cast<se_t>(std::atol(...))` to `uint32` (`phytree.cpp:232`, `common.hpp:67`). Unbounded, `{-5}` comes back as `4294967291` — breaking the promise that these numbers are echoed verbatim — and `{-1}` collides with `{4294967295}` inside krepp while looking distinct to a uniqueness check done in `int64`, reaching the `error_exit` at `:441`. `SharedKreppIndex` rejects anything outside `[0, 2^32)` up front.
  The one `error_exit` left on a non-index path is the `gzopen` of `/dev/null` in the `QSeq` that `IBatch`'s constructor requires (`rqseq.cpp:181-183`, reachable only under `EMFILE`). It now raises rather than exiting, like the rest. Same class of issue as minimap2's and abPOA's `abort()` on OOM. It is constructed once per `KreppPlacer` — i.e. once per worker thread, not once per batch — which is as narrow as the window gets without an upstream `QSeq`-free constructor.
- **Branch lengths since `31205033`:** `make_placement` (`ext/krepp/src/query.cpp:218-242`) fits `distal_length` from the read's distances to the edge's child and parent nodes when it has a finite, positive distance to the parent, falls back to `min(t/2, d_c)` when it does not, and floors `pendant_length` at 0. Before it, every placement sat at the edge midpoint and `pendant_length` was the Jukes-Cantor distance minus half the edge, negative on 270 of 404 toy-index rows. The formulas are on the columns in [`docs/phylogeny.md`](../phylogeny.md).
- **Known upstream behavior — output is not bit-reproducible.** krepp accumulates through `parallel_flat_phmap`s keyed by `node_sptr_t`, i.e. by pointer, so iteration order moves with the heap. Two consequences: `like_weight_ratio` is normalized over that order and wobbles, while `likelihood`/`distance`/`distal_length`/`pendant_length` stay bit-identical (0 differing rows across three process pairs). The numbers are in the reproducibility table in [`docs/phylogeny.md`](../phylogeny.md), which is the source of truth for them — do not restate them here. And with `multi` off, `collect_placements` (`ext/krepp/src/query.cpp:323-335`; `report_placement` does not start until :339) ranks candidates with a **non-stable** `std::sort` over a vector built from that map and takes `.back()`, so equal-ranking candidates can swap: 1 of 92 fragments alternated between two edges over 6 runs of `krepp place --no-multi`. Note the subcommand — `krepp dist` goes through `report_distances` and is a different, much noisier measurement; do not cite it for placement.
- **Building an index — why it goes out to files.** `IndexMultiple::read_input_file` takes a path and nothing else, and what it finds there decides the mode. A FASTA switches it to **per-sequence** mode, where every record is its own reference — and `obtain_build_tree` then refuses a guide tree in that mode outright (`index.cpp`: *"A guide tree (-t) is incompatible with a per sequence indexing."*). An index with a backbone can therefore only be built the other way: a TSV mapping each reference name to a file of its own. `krepp_index_create` materialises exactly that — one FASTA per reference plus the map, in a `mkdtemp` directory removed when the call ends — because it is the only shape that keeps the backbone, not because it is convenient. One file is open at a time, so descriptor cost does not scale with the reference count.
  **Name matching is the silent failure.** `build_for_subtree` walks the *tree* and looks each tip up in the map (`index.cpp`), so a reference the tree does not name is never visited: absent from the index, with no message anywhere. The other direction — a tip with no reference — prints `Genome skipped:` to stderr, which nothing in a SQL session sees. `krepp_index_create` raises on the first and warns on the second.
  The build sequence is krepp's own, in krepp's order (`krepp.cpp:517-527`): `set_nrows`, `set_lshf`, `read_input_file`, `obtain_build_tree`, `build_index`, `save_index`. It is not interchangeable — `read_input_file` decides per-sequence vs per-file and `obtain_build_tree` consults that decision.
- **Test corpus:** krepp ships no tests. `run_tests.sh` builds a toy index from krepp's own tutorial data under `ext/krepp/test/` using the parameters in its README quickstart. The stamp covers three things — the `ext/krepp` submodule HEAD, the CLI's version banner (a *different* build from the linked library, compiled separately), and the sha of the reference tarball — and each is checked for emptiness before use, since an empty component would match an empty component and silently disable the pin. The banner alone would not do: it expands `PRINT_VERSION`, a hardcoded `VERSION` string in `common.hpp` that does not change with every commit (it changed in two of the pin's last three commits, v0.9.1 → v0.9.2 → v0.10.0, none of them tagged), so two commits either side of the `generate_tree` change both report `v0.9.0` while writing incompatible indexes.
  ...
  `krepp_index_create`'s round trip needs the references but **not** the CLI — it builds in-process — so unpacking the tarball is hoisted out of the CLI-gated block and exported separately as `MIINT_KREPP_TOY_REFS`. That test therefore runs on a machine with no krepp binary at all, which is most of them while bioconda's newest krepp (0.9.1 as of 2026-09-10) is older than the pin.
